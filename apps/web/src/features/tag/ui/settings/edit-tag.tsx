...
type EditTagProps = {
  tag: TagDto;
  onSave: (newName: TagDto["name"]) => void;
  onCancel: () => void;
};

export const EditTag: FC<EditTagProps> = ({ tag, onSave, onCancel }) => {
  ...
  const { mutate: updateTag, isPending } = api.tags.updateTag.useMutation({
    onSuccess: (data) => {
      resetField("name");
      onSave(data.name);
    },
