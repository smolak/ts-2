...
import { CategoryPickerCategoriesList } from "../category-picker/category-picker-categories-list";
...
const getSelectedCategories = (categories: ReadonlyArray<CategoryDto>, searchParams: string): CategoryDto["id"][] => {
  ...
  return categories.filter(({ id }) => categoriesInSearchParams.indexOf(id) >= 0).map(({ id }) => id);
};
